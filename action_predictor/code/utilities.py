...
def load_data_set_and_sampler(config,exp_path):
    if config["graph"]["place"] == 'department':
        ...
        if config["graph"]["no_fly"] == "False":
            ...
            dataset = Decoder_Dataset(exp_path + '/../../../graph_network/graphs/ignas_big_room_wide_grid.gpickle',config)
            sampler = Sampler(exp_path + '/../../../graph_network/graphs/ignas_big_room_wide_grid.gpickle',config)
