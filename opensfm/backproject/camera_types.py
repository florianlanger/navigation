...
import numpy as np
import cv2
def load_equirectangular_camera(camera_data):
        cam = SphericalCamera()
        cam.width = camera_data['width']
        cam.height = camera_data['height']
        return cam

def get_shot_camera_data(shot_name,rdata,equi_cam):
        shot = rdata['shots'][shot_name]
        #print('shot '+str(shot))
        shot_camera = equi_cam[shot['camera']]
        pose = Pose(shot['rotation'],shot['translation'])
        #print('pose = '+str(pose))
        width = shot_camera.width
        height = shot_camera.height
        K = np.eye(3)
        R = pose.get_rotation_matrix()
        t = pose.translation
        #print('R = '+str(R))
        #print('t = '+str(t))
        return shot_camera,K,R,t
...
def denormalized_image_coordinates(norm_coords, width, height):
    size = np.float64(max(width, height))
    p = np.empty((len(norm_coords), 2))
    p[:, 0] = norm_coords[:, 0] * size - 0.5 + np.float64(width) / 2.0
    p[:, 1] = norm_coords[:, 1] * size - 0.5 + np.float64(height) / 2.0
    return p

def BackProject(x,y,depth,K,R,t):
        return np.matmul(R.T,depth*np.matmul(np.linalg.inv(K),np.array([x,y,1]))-t)
def Project(x,K,R,t):
        return np.matmul(K,np.matmul(R,x)+t)
def ProjectPixel(x,K,R,T):
        point = Project(x,K,R,t)
        px, py = np.round(point[0]/point[2]), np.round(point[1]/point[2])
        return np.int32(px),np.int32(py)
