...
def equirectangular_crop_id_image(source,params):
    ...
    return id_image,mask,mask_as_start_and_end_indices
...
def equirectangular_crop(source,params):
    ...
    return result,mask,mask_as_start_and_end_indices
...
if __name__ == "__main__":
    params={
         ...
         'VFoV_range':[0.8,2.5],
         'R': euler_to_mat(z=0,y=np.pi/2,x=0)  #range for all [0..2pi), 
        }
    ...
    for i in range(1):
        ...
        result,mask,mask_as_start_and_end_indices=equirectangular_crop(img,temp_params)
        print('result shape '+str(result.shape))
        #assert( np.sum(np.abs(result-result_slow))<0.00001)
        cv2.imwrite('./own_test/'+fname.split('.')[0]+'_C_'+str(i).zfill(6)+'.png',result)
        cv2.imwrite('./own_test/masked_image.png',mask * img)
        np.savez('mask.npz',mask=mask,mask_as_start_and_end_indices=mask_as_start_and_end_indices)
