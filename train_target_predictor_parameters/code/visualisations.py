...
def one_view_probabilities(ax,output,target,cube,text=None):
    ...
    # visualise camera
    ax.scatter(0,0,1.5,color='pink')
    ax.plot([0,0],[0,0],[0,1.5],'--',color='pink')
    ...
    print(cube[1] - 2*cube[4],cube[1] + 2*cube[4])
...
def visualise(output,target,cube,description,path):
    fig = plt.figure(figsize=plt.figaspect(0.3))
    ax1 = fig.add_subplot(1,3,1, projection='3d')
    ax1,p = one_view_probabilities(ax1,output,target,cube,description)
    fig.colorbar(p, ax=ax1)
    ax1.view_init(elev=40, azim=200)

    ax2 = fig.add_subplot(1,3,2, projection='3d')
    ax2,_ = one_view_probabilities(ax2,output,target,cube)
    ax2.view_init(elev=0, azim=180)

    ax3 = fig.add_subplot(1,3,3, projection='3d')
    ax3,_ = one_view_probabilities(ax3,output,target,cube)
...
def plot_history(file_path,store_path,epoch):
    ...
    plot_combined(['loss','dist'],[['train','val'],['train','val']],df,min_epoch,path)
