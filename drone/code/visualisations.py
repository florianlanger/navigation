...
def one_view_trajectory(ax,history_dict,folder_path):
    ...
    for i in range(history_dict["counter"]+1):
        x,y,z = history_dict['predicted_poses'][i][0],history_dict['predicted_poses'][i][1],history_dict['predicted_poses'][i][2]
        ax.scatter(x,y,z,linewidth=1,color='red',label='Predicted Poses',s=2)
        ax.text(x,y,z,str(i+1))
        dx, dy = - np.sin(2 * np.pi * x), np.cos(2 * np.pi * y)
        ax.quiver(x,y,z, dx, dy, 0, length=0.1, color='red',arrow_length_ratio=0.6)
...
def plot_trajectory(history_dict,folder_path):
    fig = plt.figure(figsize=plt.figaspect(0.3))
    ax1 = fig.add_subplot(1,3,1, projection='3d')
    ax1 = one_view_trajectory(ax1,history_dict,folder_path)

    ax2 = fig.add_subplot(1,3,2, projection='3d')
    ax2 = one_view_trajectory(ax2,history_dict,folder_path)
    ax2.view_init(elev=0, azim=270)

    ax3 = fig.add_subplot(1,3,3, projection='3d')
    ax3 = one_view_trajectory(ax3,history_dict,folder_path)
    ax3.view_init(elev=90, azim=270)
   
    fig.savefig('{}/trajectory_{}.png'.format(folder_path,str(history_dict["counter"]).zfill(2)),dpi=150)
    plt.close(fig)
...
def plot_action_predictions(history_dict,folder):
    ...
    target = history_dict['predicted_targets'][-1].round()
    ...
    ax = plot_position_and_target(ax,all_predicted_poses[i],target)
    counter = 3
    ...
    fig.savefig(folder + '/prediction_{}.png'.format(str(i).zfill(2)))
    plt.close(fig)



def plot_position_and_target(ax,position,target):
