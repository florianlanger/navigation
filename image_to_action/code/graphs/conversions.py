...
class Converter(object):
    def __init__(self,min_position,max_position,steps,number_poses,corners_no_fly_zone=None):
        ...
        assert int(self.prod_number_poses[0]) == number_poses ,"Number of poses is incorrect"
    ...
    def validate_pose(self,pose):
        if torch.all(pose + 0.0001 >= self.min_position) and torch.all(pose - 0.0001 <= self.max_position):
            return True
        else:
            return False
    ...
    def check_flyable_pose(self,pose):
        if pose.shape[0] == 3:
            ...
            return not (torch.all(pose + 0.0001 >= self.corners_no_fly_zone[0]) and torch.all(pose - 0.0001<= self.corners_no_fly_zone[1]))
