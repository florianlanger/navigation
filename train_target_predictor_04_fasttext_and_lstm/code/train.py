...
def train(model,optimizer,data_loader,config,epoch,exp_path,kind):
    total_loss = 0
    total_correct = 0
    for i,(descriptions,target_poses,descriptions_lengths) in enumerate(data_loader):

        model.zero_grad()
        pi,normal = model(descriptions,descriptions_lengths)
        loss = model.loss(descriptions,descriptions_lengths,target_poses).mean()
...
def main():
    ...
    model = MixtureDensityNetwork(config["model"]['embedding_dim'],3,config["model"]["number_modes"],ft_model,config["mode"])
