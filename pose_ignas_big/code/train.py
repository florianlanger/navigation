...
from utilities import create_directories, write_to_file
...
def one_epoch(network, config, data_loader, optimizer, epoch, exp_path,kind):
    ...
    for batch_idx, (images,positions) in enumerate(data_loader):
        optimizer.zero_grad()
...
def main():
    ...
    train_dataset = Pose_Dataset('/data/cvfs/fml35/own_datasets/grid_world/ignas_living_room_big/train','positions.csv')
    val_dataset = Pose_Dataset('/data/cvfs/fml35/own_datasets/grid_world/ignas_living_room_big/val','positions.csv')
    ...
    val_loader =  torch.utils.data.DataLoader(val_dataset, batch_size = config["training"]["batch_size"], shuffle=True)
